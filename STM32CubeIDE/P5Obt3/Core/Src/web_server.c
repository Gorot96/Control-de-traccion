...
#define SSID     "realme 6i"
#define PASSWORD "wifi0304"
#define PORT           80
...
static  WIFI_Status_t SendWebPage(uint8_t ledIsOn, struct P5sensores_t sensors);
...
static WIFI_Status_t SendWebPage(uint8_t ledIsOn, struct P5sensores_t sensors)
{
  ...
  sprintf(http, "{\"temperature\": %d, \"humidity\": %d, {\"accelX\": %d, \"accelY\": %d, \"accelZ\": %d}}",
		  (int) sensors.temperature, (int) sensors.humidity, (int) sensors.accelX, (int) sensors.accelY, (int) sensors.accelZ);
